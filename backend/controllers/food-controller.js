...
const { Food, Request } = require('../models/Foodmodel');
...
const updateReq = async (req, res) => {
  try {
    ...
    // Notify the NGO via Socket.IO
    // if (socketServer) {
    //   socketServer.to(request.ngoUsername).emit("requestResponse", {
    //     requestId,
    //     message: `Your booking request has been ${status}`,
    //     ngoUsername: request.ngoUsername,
    //     instituteUsername: request.instituteUsername,
    //   });
    // }
...
module.exports = { Add,History ,updateReq};
